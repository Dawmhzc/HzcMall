...
public interface UserMapper {
    ...
    @Update("update mall.user set user_name=#{userName}, user_pwd=#{userPwd}, user_mobile=#{userMobile}, user_email=#{userEmail} where user_id=#{userId}")
    ...
    @Insert("insert into mall.user_right(user_id, right_id) values (#{userId}, #{rightId})")
    int addUserRight(@Param("userId") int userId, @Param("rightId") int rightId);

    @Update("update mall.user_right set right_id=#{rightId} where user_id=#{userId}")
    int updateUserRight(@Param("userId") int userId, @Param("rightId") int rightId);

    @Select("select right_id from mall.user_right where user_id=#{userId}")
    ...
    @Delete("delete from mall.user_right where user_id=#{userId}")
    int deleteUserRight(int userId);
}
